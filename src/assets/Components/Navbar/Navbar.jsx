...
export default function Navbar() {
    ...
    return (
        <>
            <header className={`px-10 py-3 transition-all duration-700 ease-in-out ${menuOpen ? "bg-slate-800" : ""} ${scroll ? "bg-slate-900 fixed w-full transition-all duration-1000 ease-in" : ""}`}>
                <div className={`flex justify-between items-center`}>
                    ...
                    <Button className="text-2xl md:hidden text-white" onClick={handleClick}>
                        {!menuOpen ? <FaBarsStaggered /> : <IoMdClose />}
                    </Button>
                </div>
                ...
                {menuOpen && (
                    <nav data-aos="fade-down" data-aos-duration="600" className="text-white text-center h-60 flex items-start mt-16 md:hidden">
                        <ul className="flex flex-col text-start gap-y-8 w-full">
